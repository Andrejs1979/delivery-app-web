...
const LOGO_URI =
  "https://res.cloudinary.com/hqsczucpx/image/upload/c_scale,h_100/logo/";
...
export default function Cards({ type, data, actions }) {
  ...
  return (
    <div className="columns is-multiline is-mobile">
      {data.map(item => (
        <Item type={type} data={item} actions={actions} key={item.id} />
      ))}
    </div>
  );
}
          ...
          <div className="card">
            <div className="card-image">
              <figure className="image is-150x150">
                <img
                  src={`${CLOUDINARY}/c_scale,w_300/posts/${data.uri}`}
                  alt="Post"
                />
              </figure>
            </div>
              ...
              {actions && (
                <div className="field is-grouped">
                  <Button
                    ...
                    icon="check-circle"
                    action={() =>
                      actions[0]({ variables: { postID: data.id } })
                    }
                  >
                    Approve
                  </Button>
