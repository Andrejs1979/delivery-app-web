  ...
  return (
    <div className="control">
      <button
        ...
        onClick={action}
        disabled={disabled}
        className={`button is-${color || "primary"} is-${size ||
          "normal"} ${!block || "is-fullwidth"} ${!rounded ||
...
export function ButtonGroup({ children, attached, centered, right }) {
  return (
    <div
      className={`buttons ${!attached || "has-addons"} ${!centered ||
        "is-centered"} ${!right || "is-right"}`}
    >
      {children}
    </div>
  );
...
export function Columns({ children, centered, mobile, multiline, vertical }) {
  return (
    <div
      className={`columns ${!centered || "is-centered"} ${!vertical ||
        "is-vcentered"} ${!mobile || "is-mobile"} ${!multiline ||
        "is-multiline"}`}
    >
      {children}
    </div>
...
export function Column({ children, size, narrow }) {
  return (
    <div className={`column ${!narrow || "is-narrow"} is-${size}`}>
      {children}
    </div>
  );
}
  ...
  return (
    <span className={`icon is-${container}`}>
      <i
        className={`${
          !brand ? "fas" : "fab"
        } fa-${name} fa-${size} has-text-${color}`}
      />
    </span>
  );
}
...
export function Progress({ value, color, size }) {
  return (
    <progress
      value={value}
      max="100"
      className={`progress is-${color} is-${size}`}
    />
  );
}
    ...
    <div className="field">
      ...
      >
        <input
          ...
          className={`input is-hidden-mobile is-${size} is-${touched[
            field.name
          ] &&
            errors[field.name] &&
            "danger"}`}
        ...
        <input
          type={type}
          placeholder={placeholder}
          className={`input is-hidden-desktop is-${touched[field.name] &&
            errors[field.name] &&
            "danger"}`}
          aria-haspopup="true"
          aria-controls="suggestions"
...
export function Parent({ children, vertical }) {
  return (
    <div className={`tile is-parent ${!vertical || "is-vertical"}`}>
      {children}
    </div>
  );
}
...
export function Tile({ children, color, box }) {
  return (
    <article
      className={`tile is-child ${!color ||
        "notification"} is-${color} ${!box || "box"}`}
    >
      {children}
    </article>
  );
