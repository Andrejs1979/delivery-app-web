	...
	return (
		<section className={`hero is-${color} is-bold box`}>
				...
				<div className="hero-head">
					<nav className="level">
						...
						<div className="level-right">
							<Toolbar itemID={item} buttons={toolbar} color="primary" size="large" />
						</div>
					</nav>
				</div>
			)}
			...
			<div className="hero-foot">
				<nav className="level">
					<div className="level-left">
						{tabs && <Tabs tabs={tabs} color={color} active={activeTab} setTab={setTab} />}
					</div>

					<div className="level-right">
						{view && <ToggleViewBar view={view} color={color} setView={setView} />}
					</div>
				</nav>
			</div>
		</section>
	...
	<div className="field has-addons">
		{tabs.map((tab) => (
			<p className="control" key={tab.title}>
				<button
					className={`button is-${color} ${active === tab.title ? 'is-active' : ''}`}
					onClick={() => setTab(tab.title)}
				>
					<span className={`icon is-medium`}>
						<i className={`fas fa-${tab.icon}`} />
					</span>
					<span className={`has-text-weight-semibold is-capitalized`}>{tab.title}</span>
				</button>
			</p>
		))}
	</div>
