	...
	return (
		<div className="columns">
			<div className="column">
				<div>
					{view !== 'table' ? (
						<div className={view === 'grid' ? 'columns is-multiline is-mobile' : ''}>
							{data.map((data) => (
								<Item type={type} view={view} data={data} actions={actions} key={data.id} />
							))}
						</div>
					) : (
						<Box>
...
const TabHeader = ({ type }) => {
	switch (type) {
		case 'activity':
			return (
				<thead>
					<tr>{activity.map((header) => <th key={header.id}>{header.name}</th>)}</tr>
				</thead>
			);
		...
		case 'customers':
			return (
				<thead>
					<tr>{customers.map((header) => <th key={header.id}>{header.name}</th>)}</tr>
				</thead>
			);
		...
		case 'scheduled payments':
			return (
				<thead>
					<tr>{scheduled.map((header) => <th key={header.id}>{header.name}</th>)}</tr>
				</thead>
			);
